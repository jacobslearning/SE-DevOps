...
def test_users_page_loads(client):
    ...
    log = Log.query.filter(Log.action.contains("Users viewed")).order_by(Log.timestamp.desc()).first()
...
def test_admin_approval(client):
    ...
    log = Log.query.filter(Log.action.contains("promoted to Admin by")).order_by(Log.timestamp.desc()).first()
    assert log is not None
...
def test_user_edit(client):
    ...
    log = Log.query.filter(Log.action.contains("updated by")).order_by(Log.timestamp.desc()).first()
    assert log is not None
...
def test_user_delete(client):
    ...
    log = Log.query.filter(Log.action.contains("deleted by")).order_by(Log.timestamp.desc()).first()
    assert log is not None
...
def test_create_user(client):
    ...
    log = Log.query.filter(Log.action.contains("created by")).order_by(Log.timestamp.desc()).first()
    assert log is not None
